...
def aggregate_skill(navigator: DataNavigator, task_spec: Dict[str, Any]) -> None:
    ...
    task_spec:
        ...
        - ``operation_params.func`` – one of ``"sum"``, ``"mean"``,
          ...
          (default: ``"sum"``).
        - ``scope`` – ``"per_group"`` (default) groups by the direct parent
          ID; ``"global"`` computes a single value across all instances and
          stores it at ``navigator.data[output_property]``.
    """
